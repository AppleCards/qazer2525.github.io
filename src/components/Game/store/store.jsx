...
export const store = {
  ...
  score: observable(0), // Add score observable to keep track of the score
}
...
const updatePipe = function() {
  ...
  store.pipes.forEach( (p) => {
    ...
    else if (p.cx + pipe_w < bird.cx - bird_w / 2 && !p.passed) {
      // Bird passed through the pipe successfully
      p.passed = true;
      store.score += 1; // Increment the score when the bird passes through a pipe
    }
...
export const rungame = action(function() {
    ...
    store.score = 0; // Reset the score to 0 on game start
